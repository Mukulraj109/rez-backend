...
export class BusinessMetricsService {
  static async getDashboardMetrics(merchantId: string): Promise<DashboardMetrics> {
    ...
    const [
      ...
      OrderModel.search({ merchantId, dateRange: { start: startOfMonth, end: now } }),
      OrderModel.search({ merchantId, dateRange: { start: startOfLastMonth, end: endOfLastMonth } }),
      ProductModel.findByMerchantId(merchantId),
  ...
  static async getTimeSeriesData(merchantId: string, days: number = 30): Promise<TimeSeriesData[]> {
  ...
  static async getCategoryPerformance(merchantId: string): Promise<CategoryPerformance[]> {
    ...
    const lastMonthOrdersList = orders.filter(order => 
  ...
  static async getCustomerInsights(merchantId: string): Promise<CustomerInsights> {
    const orders = await OrderModel.findByMerchantId(merchantId);
  ...
  static async getBusinessInsights(merchantId: string): Promise<{
    insights: string[];
