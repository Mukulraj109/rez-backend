 ...
 */
async function fetchFeaturedProductsOptimized(limit: number): Promise<any[]> {
 ...
 */
async function fetchNewArrivalsOptimized(limit: number): Promise<any[]> {
  const startTime = Date.now();
  try {
 ...
 * - Reduced network overhead
 */
async function fetchStoresOptimized(featuredLimit: number, trendingLimit: number): Promise<{ featured: any[], trending: any[] }> {
  const startTime = Date.now();
  try {
    const pipeline = [
    ...
    const result = await Store.aggregate(pipeline as PipelineStage[]);
 ...
 */
async function fetchUpcomingEventsOptimized(limit: number): Promise<any[]> {
  const startTime = Date.now();
  try {
    const now = new Date();
 ...
 */
async function fetchOffersOptimized(megaLimit: number, studentLimit: number): Promise<{ mega: any[], student: any[] }> {
  const startTime = Date.now();
  try {
    const now = new Date();
    ...
    const result = await Offer.aggregate(pipeline as PipelineStage[]);
 ...
 */
async function fetchCategoriesOptimized(limit: number): Promise<any[]> {
  const startTime = Date.now();
  try {
    const pipeline = [
      ...
      {
        $sort: { productCount: -1 as -1 | 1, name: 1 as 1 | -1 }
      },
 ...
 */
async function fetchTrendingVideosOptimized(limit: number): Promise<any[]> {
  const startTime = Date.now();
  try {
    const pipeline = [
      ...
      {
        $sort: { views: -1 as -1 | 1, likes: -1 as -1 | 1 }
      },
 ...
 */
async function fetchLatestArticlesOptimized(limit: number): Promise<any[]> {
  const startTime = Date.now();
  try {
    const pipeline = [
...
export async function getHomepageDataOptimized(params: HomepageQueryParams): Promise<HomepageResponse> {
  ...
  if (needsFeaturedStores || needsTrendingStores) {
    promises.storesData = fetchStoresOptimized(
      params.limit || DEFAULT_LIMITS.featuredStores,
      params.limit || DEFAULT_LIMITS.trendingStores
    ).catch(err => {
  ...
  if (needsMegaOffers || needsStudentOffers) {
    promises.offersData = fetchOffersOptimized(
      params.limit || DEFAULT_LIMITS.megaOffers,
      params.limit || DEFAULT_LIMITS.studentOffers
    ).catch(err => {
  ...
  results.forEach((result, index) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      if (key === 'storesData') {
        if (needsFeaturedStores) data.featuredStores = value.featured;
        if (needsTrendingStores) data.trendingStores = value.trending;
      } else if (key === 'offersData') {
        if (needsMegaOffers) data.megaOffers = value.mega;
        if (needsStudentOffers) data.studentOffers = value.student;
      } else {
        data[key] = value;
      }
    }
 ...
 */
export async function comparePerformance(params: HomepageQueryParams): Promise<{
  original: { duration: number; success: boolean };
