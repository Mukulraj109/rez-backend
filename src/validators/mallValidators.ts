...
export const createBannerSchema = Joi.object({
  ...
  description: Joi.string().trim().max(1000).optional().allow(''),
  image: Joi.string().trim().uri().required(),
  position: Joi.string().valid('hero', 'middle', 'bottom', 'sidebar').default('hero'),
  ctaText: Joi.string().trim().max(50).optional(),
  ctaUrl: Joi.string().trim().uri().optional().allow(''),
  ctaBrand: Joi.string().pattern(objectIdPattern).optional(),
  ctaCategory: Joi.string().pattern(objectIdPattern).optional(),
  ctaCollection: Joi.string().pattern(objectIdPattern).optional(),
  priority: Joi.number().integer().min(0).default(0),
  isActive: Joi.boolean().default(true),
  validFrom: Joi.date().iso().required(),
  validUntil: Joi.date().iso().required(),
...
export const updateBannerSchema = Joi.object({
  ...
  description: Joi.string().trim().max(1000).optional().allow(''),
  image: Joi.string().trim().uri().optional(),
  position: Joi.string().valid('hero', 'middle', 'bottom', 'sidebar').optional(),
  ctaText: Joi.string().trim().max(50).optional(),
  ctaUrl: Joi.string().trim().uri().optional().allow(''),
