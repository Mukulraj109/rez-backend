 ...
 * @access  Super Admin
 */
router.post('/cashback-toggle', requireSuperAdmin, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[Admin Economics] Cashback toggle error:', error);
    res.status(500).json({ success: false, message: 'Failed to toggle cashback' });
  }
});
 ...
 */
router.get('/float-forecast', async (_req: Request, res: Response) => {
  try {
    ...
    logger.error('[Admin Economics] Float forecast error:', error);
