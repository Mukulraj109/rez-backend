...
class ExportService {
     ...
     * Process export job
     */
    static async processExport(job) {
        try {
     ...
     * Export sales data
     */
    static async exportSalesData(storeId, startDate, endDate) {
        const start = startDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const end = endDate || new Date();
        const salesData = await AnalyticsService_1.AnalyticsService.getSalesOverview(storeId, start, end);
        return [{
     ...
     */
    static async exportProductsData(storeId) {
        const products = await AnalyticsService_1.AnalyticsService.getTopSellingProducts(storeId, 100, 'revenue');
     ...
     */
    static async exportCustomersData(storeId) {
     ...
     * Export orders data
     */
    static async exportOrdersData(storeId, startDate, endDate) {
        const start = startDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const end = endDate || new Date();
        const salesData = await AnalyticsService_1.AnalyticsService.getSalesOverview(storeId, start, end);
        return [
