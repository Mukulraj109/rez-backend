...
exports.AuditAlertService = void 0;
...
class AuditAlertService {
     ...
     */
    static async checkAndAlert(log) {
        try {
            // Find matching rules
            ...
            // Get merchant info
            const merchant = await Merchant_1.Merchant.findById(log.merchantId);
            if (!merchant) {
     ...
     */
    static async sendAlert(merchant, log, rule) {
        try {
     ...
     * Send email alert
     */
    static async sendEmailAlert(merchant, log, rule) {
        try {
     ...
     * Send SMS alert
     */
    static async sendSMSAlert(merchant, log, rule) {
        try {
     ...
     * Format email body
     */
    static formatEmailBody(merchant, log, rule) {
        const timestamp = new Date(log.timestamp).toLocaleString();
        const user = log.merchantUserId?.name || 'System';
        return `
      <!DOCTYPE html>
      <html>
      <head>
     ...
     * Format SMS body
     */
    static formatSMSBody(merchant, log, rule) {
     ...
     */
    static async getFailedLoginCount(merchantId, since = new Date(Date.now() - 3600000) // Last hour
    ) {
        const AuditLog = (await Promise.resolve().then(() => __importStar(require('../models/AuditLog')))).default;
     ...
     */
    static async checkSuspiciousActivity(merchantId) {
        const reasons = [];
        const now = new Date();
...
exports.AuditAlertService = AuditAlertService;
// Alert rules configuration
AuditAlertService.rules = [
    {
    ...
    },
    {
        ...
        condition: (log) => log.action === 'product.bulk_deleted' &&
    ...
    },
    {
        ...
        condition: (log) => log.action === 'security.api_key_created' ||
    ...
    },
    {
        ...
        condition: (log) => log.action === 'order.status_changed' &&
    ...
    },
    {
        ...
        condition: (log) => log.action === 'store.status_changed' &&
            log.details?.after?.isActive === false,
        severity: 'high',
        notification: { email: true, sms: false }
    }
];
exports.default = AuditAlertService;
