...
class InvoiceService {
     ...
     */
    static async generateInvoice(order, merchantId) {
        try {
            ...
            const invoiceData = {
                ...
                store: {
                    ...
                    phone: store?.contactInfo?.phone || merchant.phone,
                },
            };
     ...
     */
    static async streamInvoicePDF(res, order, merchantId) {
        try {
            // Fetch merchant and store details
            const merchant = await Merchant_1.Merchant.findById(merchantId);
            if (!merchant) {
     ...
     */
    static async createInvoicePDF(data, filepath) {
        return new Promise((resolve, reject) => {
            try {
                const doc = new pdfkit_1.default({ margin: 50 });
    ...
    static addAddresses(doc, data) {
        ...
        doc.text(`${data.order.delivery.address.city}, ${data.order.delivery.address.state} ${data.order.delivery.address.pincode}`, 320, doc.y);
     ...
     */
    static async generatePackingSlip(order, merchantId) {
        try {
            this.ensureUploadDir();
            const merchant = await Merchant_1.Merchant.findById(merchantId);
            if (!merchant) {
            ...
            const filepath = path_1.default.join(process.cwd(), 'uploads', 'packing-slips', filename);
     ...
     */
    static async createPackingSlipPDF(order, merchant, store, filepath) {
        return new Promise((resolve, reject) => {
            try {
                const doc = new pdfkit_1.default({ margin: 50 });
                const stream = fs_1.default.createWriteStream(filepath);
                ...
                doc.text(`${order.delivery.address.city}, ${order.delivery.address.state} ${order.delivery.address.pincode}`, 50, doc.y);
...
InvoiceService.UPLOAD_DIR = path_1.default.join(process.cwd(), 'uploads', 'invoices');
InvoiceService.PUBLIC_URL_BASE = process.env.PUBLIC_URL || 'http://localhost:5000';
