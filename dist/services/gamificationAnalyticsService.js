...
class GamificationAnalyticsService {
    ...
    async getOverallEngagementMetrics(period = 'month') {
        const dateFilter = this.getDateFilter(period);
        const [challengeStats, achievementStats, streakStats, gameStats] = await Promise.all([
    ...
    async getChallengeEngagement(dateFilter) {
    ...
    async getAchievementEngagement(dateFilter) {
        const stats = await UserAchievement_1.default.aggregate([
            {
                $match: {
                    $or: [
    ...
    async getStreakEngagement(dateFilter) {
        const stats = await UserStreak_1.default.aggregate([
            {
                $match: { createdAt: dateFilter.createdAt || {} }
            },
            {
    ...
    async getGameEngagement(dateFilter) {
    ...
    async getUserGamificationProfile(userId) {
        const [challengeProgress, achievements, streaks, gameStats] = await Promise.all([
            UserChallengeProgress_1.default.countDocuments({ user: userId, completed: true }),
            UserAchievement_1.default.countDocuments({ user: userId, unlocked: true }),
            UserStreak_1.default.find({ user: userId }),
            GameSession_1.default.countDocuments({ user: userId, status: 'completed' })
        ]);
    ...
    async getGamificationROI(beforeDate, afterDate) {
    ...
    calculateUniqueUsers(stats) {
        const allUsers = new Set();
        stats.forEach(stat => {
            if (stat.uniqueUsers)
                allUsers.add(stat.uniqueUsers);
            if (stat.uniqueParticipants)
                allUsers.add(stat.uniqueParticipants);
            if (stat.uniquePlayers)
                allUsers.add(stat.uniquePlayers);
        });
    ...
    calculateUserLevel(stats) {
        ...
        const points = (stats.challenges * 100) +
    ...
    async getTopPerformers(limit = 10) {
        const topUsers = await UserAchievement_1.default.aggregate([
            {
                $match: { unlocked: true }
            },
            {
...
exports.default = new GamificationAnalyticsService();
