...
class TeamInvitationService {
    ...
    static async createInvitation(data) {
        try {
            ...
            const existingUser = await MerchantUser_1.MerchantUser.findOne({
                merchantId,
            ...
            const merchantUser = new MerchantUser_1.MerchantUser({
                merchantId,
                email: email.toLowerCase(),
                name,
                role,
            ...
            try {
                await this.sendInvitationEmail(email, name, merchant.businessName, role, token);
            }
            catch (emailError) {
     ...
     */
    static async resendInvitation(merchantUserId) {
        try {
            const merchantUser = await MerchantUser_1.MerchantUser.findById(merchantUserId);
            if (!merchantUser) {
                return {
                    success: false,
            ...
            const merchant = await Merchant_1.Merchant.findById(merchantUser.merchantId);
            if (!merchant) {
                return {
                    success: false,
                    message: 'Merchant not found'
            ...
            try {
                await this.sendInvitationEmail(merchantUser.email, merchantUser.name, merchant.businessName, merchantUser.role, token);
            }
            catch (emailError) {
    ...
    static async acceptInvitation(token, password) {
        try {
            ...
            const merchantUser = await MerchantUser_1.MerchantUser.findOne({
                invitationToken: hashedToken,
     ...
     */
    static async cancelInvitation(merchantUserId) {
        try {
            const merchantUser = await MerchantUser_1.MerchantUser.findById(merchantUserId);
            if (!merchantUser) {
                return {
            ...
            await MerchantUser_1.MerchantUser.deleteOne({ _id: merchantUserId });
     ...
     * Send invitation email
     */
    static async sendInvitationEmail(email, name, businessName, role, token) {
     ...
     */
    static getRolePermissionsHtml(role) {
        const permissions = {
            owner: [
    ...
    static async cleanupExpiredInvitations() {
        try {
            const result = await MerchantUser_1.MerchantUser.deleteMany({
                status: 'inactive',
...
TeamInvitationService.TOKEN_EXPIRY_HOURS = 24;
