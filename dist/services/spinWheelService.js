...
exports.selectPrize = selectPrize;
exports.spin = spin;
exports.awardSpinPrize = awardSpinPrize;
exports.getSpinHistory = getSpinHistory;
exports.getSpinStats = getSpinStats;
...
async function checkEligibility(userId) {
    const lastSpin = await MiniGame_1.MiniGame.findOne({
        user: userId,
        gameType: 'spin_wheel',
        status: 'completed'
    }).sort({ completedAt: -1 });
...
async function createSpinSession(userId) {
    ...
    const session = await MiniGame_1.MiniGame.create({
        user: userId,
        gameType: 'spin_wheel',
        status: 'active',
        expiresAt,
 ...
 */
async function spin(sessionId) {
    const session = await MiniGame_1.MiniGame.findById(sessionId);
    if (!session) {
...
async function awardSpinPrize(userId, prize) {
    if (prize.type === 'coins') {
        ...
        const { getCouponApplicability, generateCouponTitle, generateCouponDescription, generateApplicabilityText } = await Promise.resolve().then(() => __importStar(require('./spinWheelCouponAssignment')));
 ...
 */
async function getSpinHistory(userId, limit = 10) {
    const sessions = await MiniGame_1.MiniGame.find({
        user: userId,
        gameType: 'spin_wheel',
        status: 'completed'
    })
 ...
 */
async function getSpinStats(userId) {
    const sessions = await MiniGame_1.MiniGame.find({
        user: userId,
        gameType: 'spin_wheel',
        status: 'completed'
    ...
    sessions.forEach(session => {
        if (session.reward?.coins)
            totalCoinsWon += session.reward.coins;
        if (session.reward?.cashback)
            totalCashbackWon += session.reward.cashback;
        if (session.reward?.discount)
            totalDiscountsWon += session.reward.discount;
        if (session.reward?.voucher)
            totalVouchersWon += 1;
    });
