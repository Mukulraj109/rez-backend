...
exports.VoucherRedemptionService = void 0;
...
class VoucherRedemptionService {
     ...
     */
    async generateVoucher(type, amount, userId) {
     ...
     */
    generateFallbackVoucher(type, amount) {
     ...
     */
    async callVoucherAPI(provider, amount, userId) {
     ...
     */
    async claimVoucher(userId, referralId) {
     ...
     */
    async sendVoucherEmail(userId, reward) {
     ...
     */
    async checkVoucherValidity(voucherCode) {
        ...
        if (!referral)
            return false;
        const reward = referral.rewards;
        if (!reward.voucherCode || reward.voucherCode !== voucherCode)
            return false;
        if (referral.expiresAt && referral.expiresAt < new Date())
            return false;
        return true;
     ...
     */
    async getClaimableVouchers(userId) {
     ...
     */
    async getClaimedVouchers(userId) {
        const referrals = await Referral_1.default.find({
            referrer: userId,
        ...
        return claimedVouchers.sort((a, b) => (b.createdAt?.getTime() || 0) - (a.createdAt?.getTime() || 0));
    }
}
exports.VoucherRedemptionService = VoucherRedemptionService;
exports.default = new VoucherRedemptionService();
