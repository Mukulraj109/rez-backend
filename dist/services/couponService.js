...
class CouponService {
     ...
     */
    async validateCoupon(couponCode, cartData) {
        try {
            // Find coupon
            ...
            const userUsageCount = await UserCoupon_1.UserCoupon.getUserCouponUsageCount(cartData.userId, coupon._id);
     ...
     */
    async applyCouponToCart(couponCode, cartData) {
     ...
     */
    async getBestCouponForCart(cartData) {
        try {
            // Get all active coupons
            const coupons = await Coupon_1.Coupon.getActiveCoupons();
            let bestCoupon = null;
            let maxDiscount = 0;
     ...
     */
    async checkCouponApplicability(coupon, cartData) {
        // If no specific applicability rules, it's applicable to all
        if (coupon.applicableTo.categories.length === 0 &&
        ...
        if (coupon.applicableTo.userTiers.length > 0 &&
            !coupon.applicableTo.userTiers.includes('all')) {
            const user = await User_1.User.findById(cartData.userId).select('tier');
    ...
    async claimCoupon(userId, couponId) {
        try {
            ...
            const alreadyClaimed = await UserCoupon_1.UserCoupon.hasUserClaimedCoupon(userId, couponId);
            if (alreadyClaimed) {
                return {
                    success: false,
            ...
            // Create user coupon
            const userCoupon = await UserCoupon_1.UserCoupon.create({
                user: userId,
                coupon: couponId,
            ...
            await coupon.incrementClaimCount();
     ...
     */
    async markCouponAsUsed(userId, couponCode, orderId) {
        try {
            const coupon = await Coupon_1.Coupon.findOne({ couponCode: couponCode.toUpperCase() });
            if (!coupon) {
            ...
            if (userCoupon) {
                await userCoupon.markAsUsed(orderId);
            }
            // Increment coupon usage count
            await coupon.incrementUsageCount();
    ...
    async searchCoupons(query, filters = {}) {
        try {
            ...
            return await Coupon_1.Coupon.find({
                status: 'active',
                $or: [
