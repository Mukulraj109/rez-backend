...
const AnalyticsCacheService_1 = require("../merchantservices/AnalyticsCacheService");
...
async function getStoreId(req, res) {
    try {
        ...
        const store = await Store_1.Store.findOne({ merchantId }).lean();
        if (!store) {
...
router.get('/sales/overview', async (req, res) => {
    try {
        ...
        const overview = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getSalesOverviewKey(storeId, startDate, endDate), () => AnalyticsService_1.AnalyticsService.getSalesOverview(storeId, startDate, endDate), { ttl: 900 } // 15 minutes
        );
...
router.get('/sales/trends', async (req, res) => {
    try {
        ...
        const trends = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getRevenueTrendsKey(storeId, periodValue, daysValue), () => AnalyticsService_1.AnalyticsService.getRevenueTrends(storeId, periodValue, daysValue), { ttl: 900 });
...
router.get('/sales/by-time', async (req, res) => {
    try {
        ...
        if (!storeId)
            return;
        const salesByTime = await AnalyticsService_1.AnalyticsService.getSalesByTimeOfDay(storeId);
        // Ensure we always return an array
...
router.get('/sales/by-day', async (req, res) => {
    try {
        ...
        if (!storeId)
            return;
        const salesByDay = await AnalyticsService_1.AnalyticsService.getSalesByDayOfWeek(storeId);
        // Ensure we always return an array
...
router.get('/products/top-selling', async (req, res) => {
    try {
        ...
        const topProducts = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getTopProductsKey(storeId, limitValue, sortByValue), () => AnalyticsService_1.AnalyticsService.getTopSellingProducts(storeId, limitValue, sortByValue), { ttl: 1800 } // 30 minutes
        );
        // Ensure we always return an array
...
router.get('/categories/performance', async (req, res) => {
    try {
        ...
        if (!storeId)
            return;
        const categoryPerformance = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getCategoryPerformanceKey(storeId), () => AnalyticsService_1.AnalyticsService.getCategoryPerformance(storeId), { ttl: 1800 });
        // Ensure we always return an array
...
router.get('/customers/insights', async (req, res) => {
    try {
        ...
        if (!storeId)
            return;
        const customerInsights = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getCustomerInsightsKey(storeId), () => AnalyticsService_1.AnalyticsService.getCustomerInsights(storeId), { ttl: 1800 });
...
router.get('/inventory/status', async (req, res) => {
    try {
        ...
        if (!storeId)
            return;
        const inventoryStatus = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getInventoryStatusKey(storeId), () => AnalyticsService_1.AnalyticsService.getInventoryStatus(storeId), { ttl: 600 } // 10 minutes (more frequent updates for inventory)
        );
 ...
 * @access  Private
 */
router.get('/payments/breakdown', async (req, res) => {
    try {
        const storeId = await getStoreId(req, res);
        if (!storeId)
            return;
        const paymentBreakdown = await AnalyticsService_1.AnalyticsService.getPaymentMethodBreakdown(storeId);
        // Ensure we always return an array
...
router.get('/forecast/sales', async (req, res) => {
    try {
        ...
        const daysValue = parseInt(days);
        const forecast = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getSalesForecastKey(storeId, daysValue), () => PredictiveAnalyticsService_1.PredictiveAnalyticsService.forecastSales(storeId, daysValue), { ttl: 3600 } // 1 hour
        );
        // Ensure forecast has required fields - SalesForecast interface has 'forecast' array
        const forecastData = {
            forecast: forecast?.forecast ?? [],
            ...(forecast && typeof forecast === 'object' && !Array.isArray(forecast) ? forecast : {})
        };
        return res.status(200).json({
            success: true,
            data: forecastData
 ...
 * @access  Private
 */
router.get('/forecast/stockout/:productId', async (req, res) => {
    try {
        const { productId } = req.params;
        const prediction = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getStockoutPredictionKey(productId), () => PredictiveAnalyticsService_1.PredictiveAnalyticsService.predictStockout(productId), { ttl: 1800 });
        return res.json({
            success: true,
            data: prediction
        });
 ...
 * @access  Private
 */
router.get('/forecast/demand/:productId', async (req, res) => {
    try {
        const { productId } = req.params;
        const demandForecast = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getDemandForecastKey(productId), () => PredictiveAnalyticsService_1.PredictiveAnalyticsService.forecastDemand(productId), { ttl: 1800 });
        return res.json({
            success: true,
            data: demandForecast
        });
...
router.get('/trends/seasonal', async (req, res) => {
    try {
        ...
        const seasonalTrends = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getSeasonalTrendsKey(storeId, typeValue), () => PredictiveAnalyticsService_1.PredictiveAnalyticsService.analyzeSeasonalTrends(storeId, typeValue), { ttl: 3600 });
...
router.post('/cache/warm-up', async (req, res) => {
    try {
        ...
        if (!storeId)
            return;
        await AnalyticsCacheService_1.AnalyticsCacheService.warmUpCache(storeId);
        return res.json({
            success: true,
...
router.post('/cache/invalidate', async (req, res) => {
    try {
        ...
        if (!storeId)
            return;
        const count = await AnalyticsCacheService_1.AnalyticsCacheService.invalidateStore(storeId);
        return res.json({
            success: true,
...
router.get('/cache/stats', async (req, res) => {
    try {
        const stats = await AnalyticsCacheService_1.AnalyticsCacheService.getStats();
        return res.json({
            success: true,
            data: stats
        });
...
router.get('/overview', async (req, res) => {
    try {
        ...
        const [salesOverview, topProducts, customerInsights, inventoryStatus, categoryPerformance] = await Promise.all([
            AnalyticsService_1.AnalyticsService.getSalesOverview(storeId, startDate, endDate),
            AnalyticsService_1.AnalyticsService.getTopSellingProducts(storeId, 5, 'revenue'),
            AnalyticsService_1.AnalyticsService.getCustomerInsights(storeId),
            AnalyticsService_1.AnalyticsService.getInventoryStatus(storeId),
            AnalyticsService_1.AnalyticsService.getCategoryPerformance(storeId)
        ]);
        // Get revenue trends for mini chart
        const trends = await AnalyticsService_1.AnalyticsService.getRevenueTrends(storeId, 'daily', 7);
        return res.json({
            success: true,
            data: {
                sales: {
 ...
 * @access  Private
 */
router.get('/inventory/stockout-prediction', async (req, res) => {
    try {
        const storeId = await getStoreId(req, res);
        if (!storeId)
            return;
        ...
        if (productId) {
            ...
            const inventoryStatus = await AnalyticsService_1.AnalyticsService.getInventoryStatus(storeId);
            ...
            const predictions = await Promise.all(criticalProducts.map(async (item) => {
                try {
                    return await PredictiveAnalyticsService_1.PredictiveAnalyticsService.predictStockout(item.productId);
                }
                catch (error) {
                    return null;
                }
...
router.get('/products/performance', async (req, res) => {
    try {
        ...
        const topProducts = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(AnalyticsCacheService_1.AnalyticsCacheService.getTopProductsKey(storeId, limitValue, sortByValue), () => AnalyticsService_1.AnalyticsService.getTopSellingProducts(storeId, limitValue, sortByValue), { ttl: 900 });
        // Get previous period data for trend calculation (previous 30 days)
        const previousProducts = await AnalyticsService_1.AnalyticsService.getTopSellingProducts(storeId, limitValue, sortByValue, sixtyDaysAgo, thirtyDaysAgo);
    ...
    try {
        ...
        switch (groupBy) {
            case 'category':
                ...
                const prevCategoryPerformance = await AnalyticsService_1.AnalyticsService.getCategoryPerformance(storeId, sixtyDaysAgo, thirtyDaysAgo);
                ...
                break;
            case 'product':
                const topProducts = await AnalyticsService_1.AnalyticsService.getTopSellingProducts(storeId, 10, 'revenue');
                const totalRevenue = topProducts.reduce((sum, p) => sum + p.totalRevenue, 0) || 1;
                // Get previous period products for growth calculation
                const prevTopProducts = await AnalyticsService_1.AnalyticsService.getTopSellingProducts(storeId, 10, 'revenue', sixtyDaysAgo, thirtyDaysAgo);
            ...
            case 'paymentMethod':
                ...
                const prevPaymentBreakdown = await AnalyticsService_1.AnalyticsService.getPaymentMethodBreakdown(storeId, sixtyDaysAgo, thirtyDaysAgo);
...
router.get('/comparison', async (req, res) => {
    try {
        ...
        const currentPeriod = await AnalyticsService_1.AnalyticsService.getSalesOverview(storeId, startDate, endDate);
        // Previous period dates
        const prevEndDate = new Date(startDate);
        const prevStartDate = new Date(startDate);
        prevStartDate.setDate(prevStartDate.getDate() - days);
        // Get previous period data
        const previousPeriod = await AnalyticsService_1.AnalyticsService.getSalesOverview(storeId, prevStartDate, prevEndDate);
        let current, previous, change, changePercent;
        switch (metric) {
            case 'revenue':
            ...
            case 'orders':
                ...
                break;
            case 'customers':
                const currentCustomers = await AnalyticsService_1.AnalyticsService.getCustomerInsights(storeId);
                current = currentCustomers.newCustomers;
                // Get actual previous period customer insights (30-60 days ago)
                const previousCustomers = await AnalyticsService_1.AnalyticsService.getCustomerInsights(storeId, prevStartDate, prevEndDate);
...
router.get('/realtime', async (req, res) => {
    try {
        ...
        const todayMetrics = await AnalyticsCacheService_1.AnalyticsCacheService.getOrCompute(`realtime:${storeId}:${today.toISOString().split('T')[0]}`, async () => {
            const overview = await AnalyticsService_1.AnalyticsService.getSalesOverview(storeId, today, now);
            return {
        ...
        const customerInsights = await AnalyticsService_1.AnalyticsService.getCustomerInsights(storeId);
        return res.json({
            success: true,
            data: {
                ...todayMetrics,
...
router.post('/export', async (req, res) => {
    try {
        ...
        // Add job to queue
        const job = await queue_config_1.exportQueue.add(jobData, {
            attempts: 3,
            backoff: {
                type: 'exponential',
                delay: 2000
...
router.get('/export/:exportId', async (req, res) => {
    try {
        ...
        const job = await queue_config_1.exportQueue.getJob(exportId);
        if (!job) {
            return res.status(404).json({
                success: false,
