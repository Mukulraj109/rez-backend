...
const resendVerificationSchema = joi_1.default.object({
    email: joi_1.default.string().email().required()
});
...
const changePasswordSchema = joi_1.default.object({
    ...
    confirmPassword: joi_1.default.string().min(6).required().valid(joi_1.default.ref('newPassword')).messages({
        'any.only': 'Passwords do not match'
    })
});
const updateProfileSchema = joi_1.default.object({
    businessName: joi_1.default.string().min(2).max(100).optional(),
    ownerName: joi_1.default.string().min(2).max(50).optional(),
...
function generateVerificationToken(merchant) {
...
async function sendVerificationEmail(merchant, verificationToken) {
    try {
        await EmailService_1.default.sendEmailVerification(merchant.email, merchant.ownerName, verificationToken);
    }
    catch (error) {
...
router.post('/register', /* registrationLimiter, */ (0, merchantvalidation_1.validateRequest)(registerSchema), async (req, res) => {
    try {
        const { businessName, ownerName, email, password, phone, businessAddress } = req.body;
        // Normalize email to lowercase for consistency
        const normalizedEmail = email.toLowerCase().trim();
        const existingMerchant = await Merchant_1.Merchant.findOne({ email: normalizedEmail });
        if (existingMerchant) {
        ...
        const authToken = jsonwebtoken_1.default.sign({ merchantId: String(merchant._id) }, merchantSecret, { expiresIn });
...
// POST /api/auth/login
router.post('/login', /* authLimiter, */ (0, merchantvalidation_1.validateRequest)(loginSchema), async (req, res) => {
    try {
        ...
        let merchant = await Merchant_1.Merchant.findOne({ email: normalizedEmail }).select('+password');
        let merchantUser = null;
        let isOwnerLogin = false;
        // If not found as merchant, check MerchantUser table
        if (!merchant) {
            merchantUser = await MerchantUser_1.MerchantUser.findOne({ email: normalizedEmail })
                .select('+password')
                .populate('merchantId');
            if (!merchantUser) {
        ...
        const isPasswordValid = await bcryptjs_1.default.compare(password, passwordToCheck);
        if (!isPasswordValid) {
        ...
        const permissions = merchantUser ? (0, permissions_1.getPermissionsForRole)(merchantUser.role) : (0, permissions_1.getPermissionsForRole)('owner');
        const tokenPayload = {
        ...
        const token = jsonwebtoken_1.default.sign(tokenPayload, merchantSecret, { expiresIn });
        ...
        await AuditService_1.default.logAuth(String(merchant._id), 'login', {
            email,
            isOwnerLogin,
...
// @access  Public
router.post('/forgot-password', /* passwordResetLimiter, */ (0, merchantvalidation_1.validateRequest)(forgotPasswordSchema), async (req, res) => {
    try {
        const { email } = req.body;
        // Normalize email to lowercase for consistency
        const normalizedEmail = email.toLowerCase().trim();
        const merchant = await Merchant_1.Merchant.findOne({ email: normalizedEmail });
        if (!merchant) {
        ...
        merchant.resetPasswordToken = crypto_1.default.createHash('sha256').update(resetToken).digest('hex');
        merchant.resetPasswordExpiry = resetTokenExpiry;
        await merchant.save();
        // Send password reset email
        try {
            await EmailService_1.default.sendPasswordResetEmail(merchant.email, merchant.ownerName, resetToken);
        }
        catch (error) {
...
// @access  Public
router.post('/reset-password/:token', /* passwordResetLimiter, */ (0, merchantvalidation_1.validateRequest)(resetPasswordSchema), async (req, res) => {
    try {
...
router.post('/reset-password', (0, merchantvalidation_1.validateRequest)(joi_1.default.object({
...
// @access  Private
router.post('/logout', merchantauth_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (req.merchantId) {
            try {
                ...
                await AuditService_1.default.logAuth(String(req.merchantId), 'logout', auditDetails, req);
            }
            catch (auditError) {
...
router.post('/refresh', /* authLimiter, */ (0, merchantvalidation_1.validateRequest)(refreshTokenSchema), async (req, res) => {
    try {
        ...
        try {
            ...
            decoded = jsonwebtoken_1.default.verify(token, merchantSecret);
        }
        catch (error) {
            // If token is expired, verify with ignoreExpiration
            if (error.name === 'TokenExpiredError') {
                decoded = jsonwebtoken_1.default.verify(token, merchantSecret, { ignoreExpiration: true });
            }
            else {
                return res.status(401).json({
                    success: false,
        ...
        const newToken = jsonwebtoken_1.default.sign(tokenPayload, merchantSecret, { expiresIn });
        console.log(`🔄 Token refreshed for merchant: ${merchant.email}`);
        // Audit log: Token refresh
        await AuditService_1.default.logAuth(String(merchant._id), 'login', // Using login for refresh as well
        {
            type: 'token_refresh',
            email: merchant.email
        }, req);
...
router.put('/profile', merchantauth_1.authMiddleware, (0, merchantvalidation_1.validateRequest)(updateProfileSchema), async (req, res) => {
    try {
        ...
        if (updates.businessName !== undefined)
            merchant.businessName = updates.businessName;
        if (updates.ownerName !== undefined)
            merchant.ownerName = updates.ownerName;
        ...
        if (updates.logo !== undefined)
            merchant.logo = updates.logo;
        if (updates.website !== undefined)
            merchant.website = updates.website;
        if (updates.description !== undefined)
            merchant.description = updates.description;
        // Save merchant
        await merchant.save();
        // Create audit log entry
        await AuditService_1.default.logStoreChange(String(merchant._id), undefined, oldValues, {
            businessName: merchant.businessName,
            ownerName: merchant.ownerName,
            phone: merchant.phone,
            businessAddress: merchant.businessAddress,
...
router.post('/resend-verification', /* authLimiter, */ (0, merchantvalidation_1.validateRequest)(resendVerificationSchema), async (req, res) => {
    try {
        const { email } = req.body;
        // Find merchant by email
        const merchant = await Merchant_1.Merchant.findOne({ email: email.toLowerCase() })
...
router.put('/change-password', merchantauth_1.authMiddleware, (0, merchantvalidation_1.validateRequest)(changePasswordSchema), async (req, res) => {
    try {
        const merchantId = req.merchantId;
        if (!merchantId) {
            return res.status(401).json({
        ...
        const merchant = await Merchant_1.Merchant.findById(merchantId).select('+password');
        if (!merchant) {
            return res.status(404).json({
                success: false,
                message: 'Merchant not found'
        ...
        const isPasswordValid = await bcryptjs_1.default.compare(currentPassword, merchant.password);
        if (!isPasswordValid) {
            console.log(`⚠️ Failed password change attempt for merchant: ${merchant.email}`);
            // Audit log: Failed password change (best effort)
            try {
                await AuditService_1.default.logAuth(String(merchant._id), 'failed_login', {
        ...
        try {
            await AuditService_1.default.logAuth(String(merchant._id), 'password_changed', {
                email: merchant.email
            }, req);
        }
        catch (auditError) {
        ...
        try {
            await EmailService_1.default.sendPasswordChangeConfirmation(merchant.email, merchant.ownerName);
        }
        catch (error) {
...
router.post('/verify-email/:token', async (req, res) => {
    try {
        ...
        await AuditService_1.default.logAuth(String(merchant._id), 'email_verified', {
            email: merchant.email
        }, req);
        // Send welcome email
        try {
            await EmailService_1.default.sendWelcomeEmail(merchant.email, merchant.ownerName, merchant.businessName);
        }
        catch (error) {
...
router.post('/verify-email', (0, merchantvalidation_1.validateRequest)(joi_1.default.object({
    token: joi_1.default.string().required()
})), async (req, res) => {
    try {
...
async function createStoreForMerchant(merchant) {
    try {
        ...
        const merchantId = typeof merchant._id === 'string'
            ? new mongoose_1.default.Types.ObjectId(merchant._id)
            : merchant._id;
        // Check if store already exists for this merchant
        const existingStore = await Store_1.Store.findOne({ merchantId });
        if (existingStore) {
