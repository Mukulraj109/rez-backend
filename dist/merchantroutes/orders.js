...
router.get('/', async (req, res) => {
    try {
        ...
        const { status, paymentStatus, customerId, orderNumber, sortBy: sortByParam, sortOrder: sortOrderParam, page = '1', limit = '20', dateStart, dateEnd } = req.query;
...
router.put('/:id/status', async (req, res) => {
    try {
        ...
        const updatedOrder = await MerchantOrder_1.OrderModel.updateStatus(id, status, notes);
        if (!updatedOrder) {
            return res.status(500).json({
                success: false,
