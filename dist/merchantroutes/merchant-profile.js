...
const CrossAppSyncService_1 = require("../merchantservices/CrossAppSyncService");
...
router.get('/customer-view', async (req, res) => {
    try {
        ...
        const merchant = await Merchant_1.MerchantModel.findById(merchantId);
        if (!merchant) {
...
router.put('/customer-settings', async (req, res) => {
    try {
        const merchantId = req.merchantId;
        const { displayName, description, tagline, categories, tags, businessHours, deliveryOptions, serviceArea, features, socialMedia, brandColors, announcements, promotions } = req.body;
        const updates = {};
        ...
        if (deliveryOptions !== undefined)
            updates.deliveryOptions = deliveryOptions;
        ...
        if (promotions !== undefined)
            updates.activePromotions = promotions;
        const updatedMerchant = await Merchant_1.MerchantModel.findByIdAndUpdate(merchantId, updates, { new: true });
        if (!updatedMerchant) {
            return res.status(404).json({
                success: false,
                message: 'Merchant not found'
        ...
        try {
            await CrossAppSyncService_1.CrossAppSyncService.sendMerchantUpdate(merchantId, {
...
function isCurrentlyOpen(businessHours, timezone = 'UTC') {
    try {
        const now = new Date();
