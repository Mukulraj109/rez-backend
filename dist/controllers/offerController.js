...
const exclusiveOfferMiddleware_1 = require("../middleware/exclusiveOfferMiddleware");
...
const getOffers = async (req, res) => {
    try {
        ...
        const followedStores = userId ? await (0, exclusiveOfferMiddleware_1.getUserFollowedStores)(userId) : [];
        const filteredOffers = await (0, exclusiveOfferMiddleware_1.filterExclusiveOffers)(offers, userId, followedStores);
        (0, response_1.sendPaginated)(res, filteredOffers, pageNum, limitNum, total, 'Offers fetched successfully');
    }
    catch (error) {
...
const getOffersByStore = async (req, res) => {
    try {
        ...
        const filter = {
            'store.id': new mongoose_1.default.Types.ObjectId(storeId),
        };
...
const getOfferById = async (req, res) => {
    try {
        ...
        const userId = req.user?.id;
        const offerStoreId = offer.store?._id?.toString() || offer.store?.id?.toString() || offer.store?.toString();
        if (offer.isFollowerExclusive && userId) {
            const followedStores = await (0, exclusiveOfferMiddleware_1.getUserFollowedStores)(userId);
            const filteredOffers = await (0, exclusiveOfferMiddleware_1.filterExclusiveOffers)([offer], userId, followedStores);
            if (filteredOffers.length === 0) {
                return (0, response_1.sendError)(res, 'This is a follower-exclusive offer. Please follow the store to access it.', 403, { requiresFollow: true, storeId: offerStoreId });
            }
            // Record analytics for exclusive offer view
            if (offer.isFollowerExclusive && offerStoreId) {
                (0, followerAnalyticsService_1.recordExclusiveOfferView)(offerStoreId).catch(err => console.error('Failed to record exclusive offer view:', err));
            }
        }
...
const redeemOffer = async (req, res) => {
    try {
        ...
        if (offer.isFollowerExclusive) {
            ...
            if (storeId) {
                (0, followerAnalyticsService_1.recordExclusiveOfferRedemption)(storeId).catch(err => console.error('Failed to record exclusive offer redemption:', err));
            }
        }
