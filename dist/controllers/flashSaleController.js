...
const flashSaleService_1 = __importDefault(require("../services/flashSaleService"));
...
class FlashSaleController {
    ...
    async getActiveFlashSales(req, res) {
        try {
            const flashSales = await flashSaleService_1.default.getActiveFlashSales();
            res.status(200).json({
                success: true,
                data: flashSales,
     ...
     */
    async getUpcomingFlashSales(req, res) {
        try {
            const flashSales = await flashSaleService_1.default.getUpcomingFlashSales();
            res.status(200).json({
                success: true,
                data: flashSales,
                count: flashSales.length,
     ...
     */
    async getExpiringSoonFlashSales(req, res) {
        try {
            const minutes = parseInt(req.query.minutes) || 5;
            const flashSales = await flashSaleService_1.default.getExpiringSoonFlashSales(minutes);
            res.status(200).json({
                success: true,
                data: flashSales,
                count: flashSales.length,
    ...
    async getFlashSaleById(req, res) {
        try {
            const { id } = req.params;
            const flashSale = await flashSaleService_1.default.getFlashSaleById(id);
            if (!flashSale) {
                res.status(404).json({
                    success: false,
            ...
            // Track view
            await flashSaleService_1.default.trackView(id);
            res.status(200).json({
                success: true,
                data: flashSale,
            });
     ...
     */
    async getFlashSalesByProduct(req, res) {
        try {
            const { productId } = req.params;
            const flashSales = await flashSaleService_1.default.getFlashSalesByProduct(productId);
            res.status(200).json({
                success: true,
                data: flashSales,
                count: flashSales.length,
     ...
     */
    async getFlashSalesByCategory(req, res) {
        try {
            const { categoryId } = req.params;
            const flashSales = await flashSaleService_1.default.getFlashSalesByCategory(categoryId);
            res.status(200).json({
                success: true,
                data: flashSales,
                count: flashSales.length,
    ...
    async createFlashSale(req, res) {
        try {
            const flashSaleData = {
                ...req.body,
                createdBy: req.user.userId, // Assumes auth middleware sets user
            };
            const flashSale = await flashSaleService_1.default.createFlashSale(flashSaleData);
            res.status(201).json({
                success: true,
    ...
    async updateFlashSale(req, res) {
        try {
            const { id } = req.params;
            const flashSale = await flashSaleService_1.default.updateFlashSale(id, req.body);
            if (!flashSale) {
                res.status(404).json({
                    success: false,
                    message: 'Flash sale not found',
    ...
    async deleteFlashSale(req, res) {
        try {
            const { id } = req.params;
            await flashSaleService_1.default.deleteFlashSale(id);
            res.status(200).json({
                success: true,
     ...
     */
    async validateFlashSalePurchase(req, res) {
        try {
            const { flashSaleId, productId, quantity } = req.body;
            const userId = req.user.userId;
            const validation = await flashSaleService_1.default.validateFlashSalePurchase({
                flashSaleId,
                userId,
                productId,
                quantity,
    ...
    async trackClick(req, res) {
        try {
            const { id } = req.params;
            await flashSaleService_1.default.trackClick(id);
            res.status(200).json({
                success: true,
     ...
     */
    async getFlashSaleStats(req, res) {
        try {
            const { id } = req.params;
            const stats = await flashSaleService_1.default.getFlashSaleStats(id);
            if (!stats) {
                res.status(404).json({
                    success: false,
                    message: 'Flash sale not found',
     ...
     */
    async findBestOffer(req, res) {
        try {
            const { cartTotal, items } = req.body;
            const userId = req.user.userId;
            const result = await offerService_1.default.findBestOffer(cartTotal, items, userId);
            res.status(200).json({
                success: true,
                data: result,
            });
    ...
    async applyOffer(req, res) {
        try {
            ...
            const userId = req.user.userId;
            const result = await offerService_1.default.applyOffer(offerId, cartTotal, items, userId);
            if (!result.success) {
                res.status(400).json({
                    success: false,
    ...
    async validatePromoCode(req, res) {
        try {
            ...
            const userId = req.user.userId;
            const result = await offerService_1.default.validatePromoCode(promoCode, cartTotal, userId);
            if (!result.valid) {
                res.status(400).json({
                    success: false,
                    message: result.message,
