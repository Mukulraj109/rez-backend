...
exports.seedVouchersWithStores = seedVouchersWithStores;
...
async function seedVouchersWithStores() {
    try {
        ...
        const stores = await Store_1.Store.find({ isActive: true }).select('_id name slug category location');
        ...
        const existingCount = await Voucher_1.VoucherBrand.countDocuments();
        ...
        for (const voucherData of voucherBrandsData) {
            const existingVoucher = await Voucher_1.VoucherBrand.findOne({ name: voucherData.name });
            if (existingVoucher) {
                // Update existing voucher with store link if missing
                if (!existingVoucher.store && voucherData.store) {
                    existingVoucher.store = voucherData.store;
                ...
                const voucher = new Voucher_1.VoucherBrand(voucherData);
                await voucher.save();
                console.log(`✅ Created voucher brand: ${voucherData.name}${voucherData.store ? ' (linked to store)' : ''}`);
                created++;
            }
        }
        const finalCount = await Voucher_1.VoucherBrand.countDocuments();
        ...
        const vouchersWithStores = await Voucher_1.VoucherBrand.find({ store: { $exists: true, $ne: null } })
            .populate('store', 'name slug')
            .select('name store');
        console.log(`\n🔗 Vouchers linked to stores: ${vouchersWithStores.length}`);
        vouchersWithStores.forEach(v => {
            console.log(`   - ${v.name} → ${v.store?.name || 'Store'}`);
        });
