...
async function seedCoupons() {
    try {
        ...
        await UserCoupon_1.UserCoupon.deleteMany({});
        console.log('✅ Cleared existing coupons\n');
        console.log('🔄 Creating coupons...');
        // 1. Welcome Coupon - For new users
        const welcomeCoupon = await Coupon_1.Coupon.create({
        ...
        const festiveCoupon = await Coupon_1.Coupon.create({
            couponCode: 'FEST2025',
        ...
        const electronicsCoupon = await Coupon_1.Coupon.create({
            couponCode: 'TECH20',
        ...
        const storeCoupon = await Coupon_1.Coupon.create({
            couponCode: 'STORE15',
        ...
        const premiumCoupon = await Coupon_1.Coupon.create({
            couponCode: 'GOLD25',
        ...
        const productCoupon = await Coupon_1.Coupon.create({
        ...
        // 7. Weekend Special
        const weekendCoupon = await Coupon_1.Coupon.create({
        ...
        const expiredCoupon = await Coupon_1.Coupon.create({
        ...
        if (users[0]) {
            userCoupons.push(await UserCoupon_1.UserCoupon.create({
                user: users[0]._id,
        ...
        await Coupon_1.Coupon.findByIdAndUpdate(welcomeCoupon._id, { $inc: { claimCount: 1 } });
        await Coupon_1.Coupon.findByIdAndUpdate(festiveCoupon._id, { $inc: { claimCount: 1 } });
        await Coupon_1.Coupon.findByIdAndUpdate(electronicsCoupon._id, {
            $inc: { claimCount: 1, usageCount: 1, 'usageLimit.usedCount': 1 }
        });
        await Coupon_1.Coupon.findByIdAndUpdate(weekendCoupon._id, { $inc: { claimCount: 1 } });
        console.log('\n=====================================');
        ...
        console.log(`   - Available: ${userCoupons.filter((uc) => uc.status === 'available').length}`);
        console.log(`   - Used: ${userCoupons.filter((uc) => uc.status === 'used').length}`);
        console.log(`   - Expired: ${userCoupons.filter((uc) => uc.status === 'expired').length}`);
