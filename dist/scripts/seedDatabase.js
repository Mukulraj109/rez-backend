...
async function seedDatabase() {
    try {
        ...
        const createdCategories = await Category_1.Category.insertMany(categoriesData);
        ...
        if (fashionCategory) {
            storesData[0].category = fashionCategory._id;
        }
        if (foodCategory) {
            storesData[1].category = foodCategory._id;
        }
        ...
        if (fashionCategory && fashionStore) {
            productsData[0].category = fashionCategory._id;
            productsData[0].store = fashionStore._id;
        }
        if (foodCategory && pizzaStore) {
            productsData[1].category = foodCategory._id;
            productsData[1].store = pizzaStore._id;
        }
        const createdProducts = await Product_1.Product.insertMany(productsData);
        console.log(`✅ Created ${createdProducts.length} products`);
        // Seed users (optional)
        console.log('👥 Seeding users...');
        const existingUsers = await User_1.User.find({});
        if (existingUsers.length === 0) {
            const createdUsers = await User_1.User.insertMany(usersData);
