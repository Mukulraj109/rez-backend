...
async function seedQuizQuestions() {
    try {
        ...
        const deleteResult = await QuizQuestion_1.default.deleteMany({});
        console.log(`🗑️ Cleared ${deleteResult.deletedCount} existing quiz questions`);
        // Insert new quiz questions
        const result = await QuizQuestion_1.default.insertMany(quizQuestions);
        console.log(`✅ Successfully seeded ${result.length} quiz questions`);
        // Show statistics
        const stats = await QuizQuestion_1.default.aggregate([
            {
                $group: {
        ...
        // Category breakdown
        const categoryStats = await QuizQuestion_1.default.aggregate([
            {
                $group: {
                    _id: '$category',
                    count: { $sum: 1 },
...
exports.default = seedQuizQuestions;
