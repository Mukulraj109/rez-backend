...
const storeTemplatesByCategory = {
    fashion: [
        {
            name: 'Fashion Hub',
            slug: 'fashion-hub',
...
async function verifyAndLinkVoucherStores() {
    try {
        ...
        const existingStores = await Store_1.Store.find({ isActive: true });
        console.log(`✅ Found ${existingStores.length} active stores\n`);
        // Get all vouchers
        console.log('🎫 Fetching all vouchers...');
        const vouchers = await Voucher_1.VoucherBrand.find({ isActive: true });
        ...
        for (const category of Object.keys(vouchersByCategory)) {
            ...
            if (unlinkedVouchers.length > 0 && storesInCategory.length < 2) {
                ...
                if (templates.length > 0) {
                    ...
                    const categoryDoc = categories.find(c => c.slug.toLowerCase() === category ||
                        c.name.toLowerCase().includes(category)) || categories[0];
                    for (let i = storesInCategory.length; i < Math.min(2, templates.length); i++) {
                        ...
                        try {
                            ...
                            createdStoreIds[category].push(savedStore._id);
        ...
        // Refresh stores list
        const allStores = await Store_1.Store.find({ isActive: true });
        ...
        for (const voucher of vouchersWithoutStore) {
            ...
            if (availableStores.length > 0) {
                ...
                voucher.store = selectedStore._id;
                await voucher.save();
                vouchersLinked++;
        ...
        for (const voucher of vouchersWithStore) {
            const store = await Store_1.Store.findById(voucher.store);
        ...
        const finalVouchers = await Voucher_1.VoucherBrand.find({ isActive: true }).populate('store', 'name slug');
...
exports.default = verifyAndLinkVoucherStores;
