            ...
            referralCode: {
                ...
                validate: {
                    validator: function (v) {
                        ...
                        // Allow legacy 9-char codes (REF + 6 chars) for backward compatibility
                        // Prefer 11-char codes (REF + 8 chars) for new users
                        if (!v)
                            return true;
                        const validLengths = [9, 11]; // Support both old and new format
                        return validLengths.includes(v.length) && v.startsWith('REF');
                    },
                    message: 'Referral code must start with REF and be 9 or 11 characters'
                }
            },
            referredBy: {
                type: String,
...
async function generateUniqueReferralCode() {
    ...
    if (!isUnique) {
        ...
        const randomBytes = crypto_1.default.randomBytes(2).toString('hex').toUpperCase();
