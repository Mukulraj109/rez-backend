...
describe('EmailService', () => {
    ...
    describe('send', () => {
        it('should send email successfully when SendGrid is configured', async () => {
            ...
            await EmailService_1.default.send(options);
        ...
        it('should log to console when SendGrid is not configured', async () => {
            ...
            await EmailService_1.default.send(options);
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('EMAIL (SendGrid not configured'));
            expect(mail_1.default.send).not.toHaveBeenCalled();
            consoleSpy.mockRestore();
        });
        it('should handle email sending errors', async () => {
            process.env.SENDGRID_API_KEY = 'SG.test_key_123456789';
            mail_1.default.send.mockRejectedValueOnce(new Error('SendGrid API error'));
            const options = {
                to: 'test@example.com',
                subject: 'Test Email',
                text: 'Test email body'
            };
            await expect(EmailService_1.default.send(options)).rejects.toThrow('Failed to send email');
        });
    ...
    describe('sendPasswordChangeConfirmation', () => {
        it('should send password change confirmation email', async () => {
            process.env.SENDGRID_API_KEY = 'SG.test_key_123456789';
            await EmailService_1.default.sendPasswordChangeConfirmation('merchant@test.com', 'Test Merchant');
            expect(mail_1.default.send).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should include merchant name in email', async () => {
            process.env.SENDGRID_API_KEY = 'SG.test_key_123456789';
            await EmailService_1.default.sendPasswordChangeConfirmation('merchant@test.com', 'Test Merchant');
            const callArgs = mail_1.default.send.mock.calls[0][0];
    ...
    describe('sendRefundConfirmation', () => {
        it('should send refund confirmation email', async () => {
            ...
            await EmailService_1.default.sendRefundConfirmation('customer@test.com', 'Test Customer', refundDetails);
            expect(mail_1.default.send).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('sendOnboardingSubmitted', () => {
        it('should send onboarding submission confirmation', async () => {
            process.env.SENDGRID_API_KEY = 'SG.test_key_123456789';
            await EmailService_1.default.sendOnboardingSubmitted('merchant@test.com', 'Test Merchant');
            expect(mail_1.default.send).toHaveBeenCalledWith(expect.objectContaining({
                to: 'merchant@test.com',
