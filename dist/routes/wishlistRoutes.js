...
router.get('/default', 
// generalLimiter,, // Disabled for development
wishlistController_1.getDefaultWishlist);
// Check if item is in wishlist (must be before /:wishlistId)
router.get('/check', 
// generalLimiter,, // Disabled for development
(0, validation_1.validateQuery)(validation_2.Joi.object({
    itemType: validation_2.Joi.string().valid('product', 'store', 'video', 'Product', 'Store', 'Video').required(),
    itemId: validation_1.commonSchemas.objectId().required()
})), wishlistController_1.checkWishlistStatus);
...
(0, validation_1.validate)(validation_2.Joi.object({
    itemType: validation_2.Joi.string().valid('product', 'store', 'video', 'Product', 'Store', 'Video').required(),
    itemId: validation_1.commonSchemas.objectId().required()
})), wishlistController_1.removeItemByTypeAndId);
