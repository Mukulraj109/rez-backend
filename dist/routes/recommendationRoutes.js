...
const diverseRecommendationController_1 = require("../controllers/diverseRecommendationController");
...
auth_1.optionalAuth, (0, validation_1.validateBody)(validation_2.Joi.object({
    excludeProducts: validation_2.Joi.array().items(validation_2.Joi.string().pattern(/^[0-9a-fA-F]{24}$/)).optional(),
    excludeStores: validation_2.Joi.array().items(validation_2.Joi.string().pattern(/^[0-9a-fA-F]{24}$/)).optional(),
    shownProducts: validation_2.Joi.array().items(validation_2.Joi.string().pattern(/^[0-9a-fA-F]{24}$/)).optional(),
    limit: validation_2.Joi.number().integer().min(1).max(50).default(10),
    context: validation_2.Joi.string().valid('homepage', 'product_page', 'store_page', 'category_page').default('homepage'),
    options: validation_2.Joi.object({
        minCategories: validation_2.Joi.number().integer().min(1).max(10).optional(),
        maxPerCategory: validation_2.Joi.number().integer().min(1).max(10).optional(),
        maxPerBrand: validation_2.Joi.number().integer().min(1).max(10).optional(),
        diversityScore: validation_2.Joi.number().min(0).max(1).optional(),
        includeStores: validation_2.Joi.boolean().optional(),
        algorithm: validation_2.Joi.string().valid('hybrid', 'collaborative', 'content_based').default('hybrid'),
        minRating: validation_2.Joi.number().min(0).max(5).optional(),
        priceRanges: validation_2.Joi.number().integer().min(2).max(5).default(3)
    }).optional()
})), diverseRecommendationController_1.getDiverseRecommendations);
