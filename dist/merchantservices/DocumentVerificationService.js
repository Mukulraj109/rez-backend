...
class DocumentVerificationService {
     ...
     */
    static async uploadDocument(file, merchantId, documentType) {
        try {
     ...
     */
    static async addDocumentToOnboarding(merchantId, documentType, documentUrl) {
     ...
     */
    static async getMerchantDocuments(merchantId) {
        const merchant = await Merchant_1.Merchant.findById(merchantId);
        if (!merchant) {
            throw new Error('Merchant not found');
        }
     ...
     */
    static async verifyDocument(merchantId, documentIndex, adminId, approved, rejectionReason) {
        const merchant = await Merchant_1.Merchant.findById(merchantId);
        if (!merchant) {
            throw new Error('Merchant not found');
        }
        ...
        await this.sendDocumentVerificationEmail(merchant, documents[documentIndex], approved);
        return {
     ...
     */
    static async verifyAllDocuments(merchantId, adminId, approved, rejectionReason) {
        const merchant = await Merchant_1.Merchant.findById(merchantId);
        if (!merchant) {
            throw new Error('Merchant not found');
        }
        ...
        await merchant.save();
        // Send notification email
        await EmailService_1.EmailService.sendDocumentVerificationComplete(merchant.email, merchant.ownerName, approved, rejectionReason);
        return {
            success: true,
     ...
     */
    static async requestAdditionalDocuments(merchantId, documentTypes, message) {
        const merchant = await Merchant_1.Merchant.findById(merchantId);
        if (!merchant) {
            throw new Error('Merchant not found');
        }
        // Send email to merchant
        await EmailService_1.EmailService.sendAdditionalDocumentsRequest(merchant.email, merchant.ownerName, documentTypes, message);
        return {
            success: true,
     ...
     * Delete a document
     */
    static async deleteDocument(merchantId, documentIndex) {
        const merchant = await Merchant_1.Merchant.findById(merchantId);
        if (!merchant) {
            throw new Error('Merchant not found');
        }
     ...
     */
    static async extractDocumentData(documentUrl, documentType) {
     ...
     */
    static async validateDocumentAuthenticity(documentType, documentNumber) {
     ...
     */
    static async sendDocumentVerificationEmail(merchant, document, approved) {
        ...
        if (approved) {
            await EmailService_1.EmailService.sendDocumentApproved(merchant.email, merchant.ownerName, documentName);
        }
        else {
            await EmailService_1.EmailService.sendDocumentRejected(merchant.email, merchant.ownerName, documentName, document.rejectionReason || 'Document does not meet requirements');
        }
    }
    /**
...
exports.default = DocumentVerificationService;
