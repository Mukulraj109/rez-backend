...
class BusinessMetricsService {
    static async getDashboardMetrics(merchantId, storeId) {
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        const startOfLastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
        const endOfLastMonth = new Date(now.getFullYear(), now.getMonth(), 0);
        // Build search criteria with optional storeId filter
        const baseSearchCriteria = { merchantId };
        if (storeId) {
        ...
        const [orders, monthlyOrdersResult, lastMonthOrdersResult, products, cashbackRequests] = await Promise.all([
            ...
            MerchantOrder_1.OrderModel.search({ ...baseSearchCriteria, dateRange: { start: startOfMonth, end: now } }),
            MerchantOrder_1.OrderModel.search({ ...baseSearchCriteria, dateRange: { start: startOfLastMonth, end: endOfLastMonth } }),
            MerchantProduct_1.ProductModel.findByMerchantId(merchantId, storeId),
        ...
        if (storeId) {
            ...
            filteredOrders = orders.filter(order => order.items.some(item => productIds.has(item.productId)));
            filteredMonthlyOrders = monthlyOrdersResult.orders.filter((order) => order.items.some((item) => productIds.has(item.productId)));
            filteredLastMonthOrders = lastMonthOrdersResult.orders.filter((order) => order.items.some((item) => productIds.has(item.productId)));
        }
        ...
        const completedOrdersWithTimes = filteredOrders.filter(order => order.status === 'delivered' && order.createdAt && order.updatedAt);
    ...
    static async getTimeSeriesData(merchantId, days = 30, storeId) {
    ...
    static async getCategoryPerformance(merchantId, storeId) {
        ...
        const lastMonthOrdersList = filteredOrders.filter(order => order.createdAt >= startOfLastMonth && order.createdAt <= endOfLastMonth);
    ...
    static async getCustomerInsights(merchantId, storeId) {
        let orders = await MerchantOrder_1.OrderModel.findByMerchantId(merchantId);
        // Filter orders by products' storeId if storeId is provided
        if (storeId) {
            const products = await MerchantProduct_1.ProductModel.findByMerchantId(merchantId, storeId);
            const productIds = new Set(products.map((p) => p._id.toString()));
            orders = orders.filter(order => order.items.some(item => productIds.has(item.productId)));
        }
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    ...
    static async getBusinessInsights(merchantId, storeId) {
